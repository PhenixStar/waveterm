...
Use `tsh ai` to send files and prompts from the command line:

```bash
git diff | tsh ai -                          # Pipe to AI
tsh ai main.go -m "find bugs"               # Attach files with message
tsh ai $(tail -n 500 my.log) -m "review" -s # Auto-submit with output
```
...
Terminolgy AI supports your own AI models and API keys:
...
**Default Terminolgy AI Service:**
- Messages are proxied through the Terminolgy Cloud AI service (powered by OpenAI's APIs). Please refer to OpenAI's privacy policy for details on data handling.
- Terminolgy does not store your chats, attachments, or use them for training
...
- Refer to your provider's privacy policy for data handling details

<Info>
  **Under Active Development**

  Terminolgy AI is in active beta with included AI credits while we refine the experience. Share feedback in [Discord](https://discord.gg/XfvZ334gwU).
