...
## What it is
- Electron + React front end with a Go backend ("terminolgy-srv"). Provides a terminal with GUI widgets, previews, web, and AI. (README.md)
...
- **Go backend ("terminolgy-srv"):** starts services, web and websocket listeners, telemetry loops, config watcher, local RPC, filestore and SQLite-backed object store. (\`cmd/server/main-server.go\`, \`pkg/*\`)
- **CLI/helper ("tsh"):** built for multiple OS/arch; used for shell integration and remote operations. (\`cmd/wsh/\`, \`Taskfile.yml build:wsh\`)

## Key directories
- **cmd/:** entrypoints and generators
  - \`server/\`: terminolgy-srv main
